...
function App() {
  ...
  return (
    <div>
      <input type="text" name="" id="" placeholder='Type todo' value={todo} onChange={(e)=> setToDo(e.target.value)}/>
      <button onClick={handleAddTodo}>{isEditing ? 'Update ToDo' : 'Add To ToDos'}</button>
      <div>
        {todos.map((todo, index) => (
          <div>
          <span onDoubleClick={() => handleDoubleClick(index)} style={{ fontStyle: italicIndex === index ? 'italic': 'normal'}}>{todo}</span>
          <button onClick={() => handleEdit(index)}>Edit</button>
          <button onClick={() => handleDelete(index)}>Delete</button>
          </div>
        ))}
